...
export async function initializeDatabase() {
    ...
    `);
        const [questionTextColumns] = await connection.query(`SELECT COLUMN_NAME FROM information_schema.COLUMNS
       WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'questions' AND COLUMN_NAME = 'question_text'`, [databaseName]);
        if (!questionTextColumns.length) {
            await connection.query('ALTER TABLE questions ADD COLUMN question_text TEXT NULL AFTER question_number');
        }
        const [imagePathColumns] = await connection.query(`SELECT IS_NULLABLE FROM information_schema.COLUMNS
       WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'questions' AND COLUMN_NAME = 'image_path'`, [databaseName]);
      ...
      CONSTRAINT fk_attempts_test FOREIGN KEY (test_id) REFERENCES tests(id) ON DELETE CASCADE, CONSTRAINT uq_student_test_attempt UNIQUE (test_id, student_id)
        ...
        const [attemptColumns] = await connection.query(`SELECT COLUMN_NAME FROM information_schema.COLUMNS
       WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'test_attempts' AND COLUMN_NAME = 'remaining_time_seconds'`, [databaseName]);
        if (!attemptColumns.length) {
            await connection.query('ALTER TABLE test_attempts ADD COLUMN remaining_time_seconds INT UNSIGNED NOT NULL DEFAULT 0 AFTER expires_at');
        }
